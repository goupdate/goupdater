...
var (
	...
	validFor   = flag.Duration("duration", 30*365*24*time.Hour, "Duration that certificate is valid for")
...
//gencert --host=localost -rsa-bits=4096 -ecdsa-curve=P521 -duration=1000000h
func Generate() {
	flag.Parse()
	...
	if err := certOut.Close(); err != nil {
		log.Fatalf("Error closing tls.cert: %v", err)
	}
	log.Print("wrote tls.cert\n")

	keyOut, err := os.OpenFile("tls.key", os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0600)
	if err != nil {
		log.Fatalf("Failed to open tls.key for writing: %v", err)
		return
	}
	...
	if err := keyOut.Close(); err != nil {
		log.Fatalf("Error closing tls.key: %v", err)
	}
